...
function renderScene() {
  ...
  // --- 一般場景：預設渲染 ---
  if (sceneText) {
    // ===== 修改：第 1-6 關開場影片已播放，跳過第一個場景嘅開場文字描述 =====
    const curLevel = getCurrentLevel();
    const isFirstScene = curLevel && curLevel.scenes && curLevel.scenes[0] && curLevel.scenes[0].id === scene.id;
    if (gameState._skipOpeningText && isFirstScene) {
      sceneText.innerHTML = ''; // 由影片取代開場文字
      gameState._skipOpeningText = false; // 只影響第一個場景
    } else {
...
// ===================================================================
// ===== 關卡開場影片（第 1-6 關）=====
// 說明：進入第 1-6 關時，先播放全螢幕 MP4 劇情影片，取代原本嘅開場文字描述。
//       影片結束 / 使用者跳過 / 載入失敗後，先繼續原本嘅 startLevel 渲染流程。
// ===================================================================

/**
 * 播放關卡開場影片。
 * @param {number} levelNumber 關卡編號（1-6）
 * @param {Function} onComplete 影片結束（或跳過/失敗）後嘅回調
 */
function playLevelIntroVideo(levelNumber, onComplete) {
  const overlay = document.getElementById('levelVideoOverlay');
  const video = document.getElementById('levelVideoPlayer');
  const skipBtn = document.getElementById('levelVideoSkipBtn');
  const playBtn = document.getElementById('levelVideoPlayBtn');

  // 若 DOM 不存在，直接繼續（容錯）
  if (!overlay || !video) { onComplete && onComplete(); return; }

  let finished = false;

  // 結束處理：隱藏影片 DOM，釋放資源，執行回調
  const finishVideo = () => {
    if (finished) return;
    finished = true;
    try {
  ...
  // 設定影片來源（assets/videos/{n}.mp4）
  video.src = 'assets/videos/' + levelNumber + '.mp4';

  // 嘗試播放（此時通常喺使用者點擊「下一關」/選擇後嘅 user gesture 鏈內）
  const playPromise = video.play();
  if (playPromise && typeof playPromise.catch === 'function') {
    playPromise.catch(() => {
      // 瀏覽器擋咗自動播放 → 顯示中央「點擊播放」按鈕，play() 放入 click 回調內
  ...
  // 載入失敗 → 直接跳過，執行原本關卡邏輯（容錯）
  video.onerror = () => {
    console.warn('[level-video] 影片載入失敗，跳過影片：', video.src);
    finishVideo();
  };
}
...
function startLevel() {
  ...
  const levelNumber = gameState.currentLevelIndex + 1; // 1-based

  // ===== 修改：第 1-6 關先播放開場影片，再渲染場景 =====
  if (levelNumber >= 1 && levelNumber <= 6) {
    // 標記：渲染第一個場景時跳過開場文字描述（由影片取代）
    gameState._skipOpeningText = true;
    playLevelIntroVideo(levelNumber, () => {
      startLevelRenderScene();
    });
    return;
  }

  // 第 7 關及其他關卡：維持原本流程
  gameState._skipOpeningText = false;
  startLevelRenderScene();
}

// 原本 startLevel 嘅渲染邏輯抽出嚟，方便影片回調後呼叫
function startLevelRenderScene() {
  const level = getCurrentLevel();
  if (!level || !level.scenes || level.scenes.length === 0) return;
...
function restartGame() {
  ...
  gameState._skipOpeningText = false; // 重置開場影片跳過文字標記
  stopCountdown();
